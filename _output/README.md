...
| 当前工作树 | `0.8.0` | ICM45686 正式启用与真实启动状态日志 |

这些映射用于描述源码演进，不改写历史文件中已经固化的镜像头、文件名或实物证据。当前
目标板 confirmed 版本已通过 UART OTA 更新为 `0.8.0 build1`；本轮只在 `build/` 生成临时
OTA 包，尚未向 `_output/application/` 存放正式产物。后续整理产物时不得覆盖 b12 factory 文件。
