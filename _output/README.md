...
| 上一工作树 | `0.4.0` | LCD 小 Logo 与页面结构调整 |
| 上一修复树 | `0.5.0` | LCD 四页状态显示，DMA 调度过慢 |
| 上一修复树 | `0.5.1` | LCD 四页状态显示与 DMA 调度修复 |
| 上一工作树 | `0.5.2` | LCD 任务诊断周期统一 |
| 上一工作树 | `0.6.0` | LCD UI 美化、透明 Logo 和电量估算 |
| 上一工作树 | `0.7.0` | LCD 信息层级、大号电量显示和双栏布局 |
| 上一工作树 | `0.7.1` | LCD 中性深灰背景和短标题强调线 |
...
confirmed 版本已于 2026-08-19 通过 UART OTA 更新为 `0.14.0 build1`；匹配 OTA 包仍保留在
...
`app-v0.15.0-b1.ota` 仍只放在 `build/arm-release/`：当前 DT/device data 第一批收口后的
payload 为 106648 字节、CRC32 `0xE60C022C`，OTA 包为 106712 字节。0.15.0 尚未烧录，也未复制到
`_output/application/`。
