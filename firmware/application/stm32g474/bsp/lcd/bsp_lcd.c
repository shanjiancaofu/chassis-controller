...
bool BspLcd_BeginFrame(void)
{
  ...
  lcd_dma_failed = false;
  lcd_dma_pending = false;
  lcd_status = BSP_LCD_TRANSMITTING;
  return true;
}

bool BspLcd_TransmitRow(const uint8_t *row_data, uint16_t size)
{
  if (row_data == NULL || size != BSP_LCD_WIDTH * 2U ||
      lcd_status != BSP_LCD_TRANSMITTING || lcd_dma_pending) {
    return false;
  }
  lcd_dma_complete = false;
...
{
  return lcd_status == BSP_LCD_TRANSMITTING && lcd_dma_complete &&
         !lcd_dma_pending;
}

bool BspLcd_HasTransferError(void)
{
  return lcd_dma_failed || lcd_status == BSP_LCD_FAILED;
}

void BspLcd_EndFrame(void)
{
  if (lcd_status != BSP_LCD_TRANSMITTING || lcd_dma_pending) {
    lcd_status = BSP_LCD_FAILED;
    return;
  }
  HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_SET);
