...
{
  (void)device;
  return UartStm32WriteTracked(data, length, token);
}
...
{
  (void)device;
  return UartStm32GetTrackedCompletion(token, completed, success);
}

static bool IsTxIdle(const struct device *device)
{
  (void)device;
  return UartStm32IsTxIdle();
}

static uint8_t GetTxSlotsAvailable(const struct device *device)
{
  (void)device;
  return UartStm32GetTxSlotsAvailable();
}

static void GetDiagnostics(const struct device *device,
                           UartDiagnostics *diagnostics)
{
  UartStm32Diagnostics bsp_diagnostics;
  (void)device;
  UartStm32GetDiagnostics(&bsp_diagnostics);
  if (diagnostics == NULL) {
    return;
  }
