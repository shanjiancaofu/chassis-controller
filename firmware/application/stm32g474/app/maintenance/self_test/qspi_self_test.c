...
bool QspiSelfTest_Start(void)
{
  ...
  if (QspiSelfTest_GetStatus() == QSPI_SELF_TEST_RUNNING ||
...
void QspiSelfTest_Run(uint32_t now_ms)
{
  ...
  switch (test_state) {
    case TEST_ERASE_START:
      if (flash_erase_sector(QSPI_TEST_START)) {
    ...
    case TEST_ERASE_READ_START:
      if (flash_read_dma(QSPI_TEST_START, read_buffer,
                               sizeof(read_buffer))) {
...
QspiSelfTestStatus QspiSelfTest_GetStatus(void)
{
  if ((test_state >= TEST_ERASE_START && test_state <= TEST_VERIFY) ||
    ...
    return QSPI_SELF_TEST_RUNNING;
  }
  if (test_state == TEST_PASSED) {
    return QSPI_SELF_TEST_PASSED;
  }
  if (test_state == TEST_FAILED) {
    return QSPI_SELF_TEST_FAILED;
  }
  return QSPI_SELF_TEST_IDLE;
}
