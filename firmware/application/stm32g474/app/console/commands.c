...
bool ConsoleCommands_Init(const ConsoleCommandPort *port)
{
  if (port == NULL || port->submit_motion_command == NULL ||
      ...
      port->acquire_self_test_lock == NULL ||
      port->start_motor_self_test == NULL) {
    return false;
  }
  command_port = *port;
  return true;
                                    ...
                                    uint32_t now_ms)
{
  ...
  switch (command->type) {
    ...
    case CONSOLE_COMMAND_MOTOR_RIGHT_REVERSE: {
      const MotorSelfTestAction action =
      ...
      if (action == MOTOR_SELF_TEST_STOP) {
        ...
        accepted = command_port.acquire_self_test_lock() &&
                   command_port.start_motor_self_test(action, now_ms);
        if (!accepted) {
          kernel_critical_enter();
          CommandManager_Release(COMMAND_SOURCE_SELF_TEST);
          kernel_critical_exit();
      ...
      (void)UartMessages_SendResponse(
          ...
          action == MOTOR_SELF_TEST_STOP
              ? "motor_stop"
              : action == MOTOR_SELF_TEST_LEFT_FORWARD
                    ? "motor_left_forward"
              : action == MOTOR_SELF_TEST_LEFT_REVERSE
                          ? "motor_left_reverse"
                          : action == MOTOR_SELF_TEST_RIGHT_FORWARD
                                ? "motor_right_forward"
                                : "motor_right_reverse",
          accepted,
          accepted ? (action == MOTOR_SELF_TEST_STOP ? "state=STOPPED"
                                                       : "state=STARTED")
                   : "code=BUSY");
      break;
    }
