#ifndef CONTROL_CONFIG_H
#define CONTROL_CONFIG_H

#define MOTOR_CONTROL_PERIOD_MS 1U

/* Keep commands, PID gains, and telemetry in the validated 10 ms encoder-count unit. */
#define MOTOR_CONTROL_REFERENCE_PERIOD_MS 10U

/* Closed-loop limit: 80% of the 8500-count TIM8 PWM period. */
#define MOTOR_CONTROL_OUTPUT_LIMIT 8000
#define MOTOR_CONTROL_TARGET_LIMIT 100
#define MOTOR_CONTROL_TARGET_SLEW_COUNTS_PER_REFERENCE_PERIOD 5
#define MOTOR_ENCODER_MAX_DELTA_PER_REFERENCE_PERIOD 500
#define MOTOR_CONTROL_MIN_SUPPLY_MV 9000U
#define MOTOR_CONTROL_MAX_SUPPLY_SAMPLE_AGE_MS 250U
#define MOTOR_ENCODER_FEEDBACK_LOSS_TIMEOUT_MS 500U
#define MOTOR_ENCODER_STARTUP_OUTPUT_THRESHOLD 3500
#define MOTOR_STARTUP_BOOST_DUTY 3500
#define MOTOR_STARTUP_BOOST_MAX_MS 150U
#define MOTOR_ENCODER_COUNTS_PER_REVOLUTION 1320
#define CHASSIS_WHEEL_DIAMETER_M 0.065f
#define CHASSIS_TRACK_WIDTH_M 0.220f

/* Initial values; finish tuning with the drive wheels suspended. */
#define MOTOR_LEFT_PID_KP 200.0f
#define MOTOR_LEFT_PID_KI 300.0f
#define MOTOR_LEFT_PID_KD 0.0f
#define MOTOR_RIGHT_PID_KP 250.0f
#define MOTOR_RIGHT_PID_KI 400.0f
#define MOTOR_RIGHT_PID_KD 0.0f
#define MOTOR_PID_KP_MAX 1000U
#define MOTOR_PID_KI_MAX 5000U
#define MOTOR_PID_KD_MAX 100U

#endif
