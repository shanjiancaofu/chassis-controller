...
开发板原理图虽然将 `PF2` 标为 `EXT_ADC_CH1`，但 STM32G474VETx 官方引脚数据库表明 `PF2` 没有 ADC 常规输入通道，不能用于模拟采样；`PA0` 标为 `EXT_ADC_CH2`，但当前已用于左编码器。后续电压和电流采样应从 `PA2`、`PA3`、`PC3` 等真正支持 ADC 且空闲的引脚中选择，并按外部采样电路确定量程。
