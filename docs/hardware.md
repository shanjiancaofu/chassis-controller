...
硬件和引脚变更先修改 `firmware/application/stm32g474/cubemx/chassis_controller.ioc`，生成并
检查无冲突后再同步本文。
...
| LCD | 2 英寸 `ZJY200K-IF05`，ST7789V，面板 240(H) x 320(V)，当前横屏显示 320 x 240 |
...
当前标定为 `1320 counts/rev`。2026-08-19 实测几何输入为轮胎有效直径 `65 mm`、左右驱动轮
接地点中心轮距 `220 mm`；据此每个编码器 count 对应约 `0.1547 mm` 轮缘行程。上述尺寸是当前
轮式里程计初始参数，落地直线和原地旋转实测后仍需校准；更换轮胎、减速比、编码器或计数配置
后必须重新标定。
...
该值用于状态观测、电量窗口估算，并由控制安全路径读取诊断任务缓存值；当前低于 `9000 mV`
会锁存欠压 critical fault 并急停。该阈值尚未通过可控欠压注入校准，不实现过压保护。
...
不按时触点断开，内部上拉使 PD2 为高，表示正常；按下时触点接通 GND，PD2 变低并触发急停。
固件使用低有效和 EXTI2 下降沿。按钮不连接 3.3 V 或 5 V。

## LCD

当前屏幕参考资料为 `屏幕资料/2.00寸/ZJY200K-IF05.pdf`、
`屏幕资料/2.00寸/2.00 寸原理图.pdf` 和 `屏幕资料/2.00寸/ST7789芯片手册.pdf`。
资料中的面板原生方向为 240 x 320；驱动通过 ST7789 `MADCTL` 旋转后按 320 x 240 横屏写入。
同目录的 1.47 寸 `ZJY147TC-IF15` 是 172 x 320、ST7789V3，不属于当前 2 寸屏配置，不能直接
复用当前分辨率和窗口参数。
