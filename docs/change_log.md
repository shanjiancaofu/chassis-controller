...
- 主电源约 12.05V、车轮架空、未移动线束，分别测试 `100/200/300/500 mm/s`、每级约 0.3s，
  各级结束发零速。
- `100 mm/s` 时左右最大 PWM 约 `218/266 permille`，左轮速度 0、右轮约 15；`200 mm/s` 开始
...
- 电机主电源接通、车轮架空后执行约 `+100 mm/s`、1.2 秒低速前进；CAN 发送/接收正常，STM32
  保持 RUNNING、无 fault，动作结束已发零速并回到 `STOPPED/PWM=0`。
- 右编码器累计约 73 counts，左编码器保持 0；该结果不能证明双轮闭环、方向或里程计通过。
- 按安全策略暂停后退、旋转和更高速度测试；不动线束。下一步先检查左编码器磁环/传感器供电与安装间隙，
  修复后再继续 wheels-up Gate。
...
- `WheelController_Update()` 对 `elapsed_ticks==0` 立即返回 `false`，避免无效 dt 进入 slew/PID。
- host 单测新增零 tick 拒绝断言；Release 和全量 host tests 通过。
...
- 新增轻量 `tools/test/run_all.py`、`test_matrix.yaml` 和四场景配置矩阵，支持 JSON/JUnit；IMU+DTS disabled 和
  SelfTest disabled 均完成真实 Application build。
- Debug/Release、14 项 C 主机测试、18 项 OTA/Python、3 项 schema、Kconfig/DTS、架构检查和
  LCD 预览通过；未烧录、未新增硬件结论。
