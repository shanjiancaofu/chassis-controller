...
迁移状态（2026-08-24）：软件架构收敛已完成。DT/device data 第一批已完成：公共
...
| `app/ui/lcd/` 与 `drivers/display/` | UI 持有布局/像素，driver 持有控制器/SPI DMA | 已完成拆分 |
| `app/diagnostics/system_status_collector.c` | 统一组装 driver/RTOS 到诊断 DTO | 0.15.0 已完成拆分 |
...
| 正式稳定基线 | 首个 1.x 基线 | `1.0.0 build1` | UART OTA、普通复位、静态零 PWM、主要外设和急停锁存已复核 |
| 当前板上/源码 | PB8 与目标板回归修复 | `1.0.1 build1` | UART OTA confirmed；自动串口回归和 PB8 实际换页通过 |

### 阶段 3：LCD 硬件总览

状态：`HARDWARE PASS`（1.0.1 build1 已上板报告 LCD READY，PB8 实际循环换页通过；动态页面
视觉沿用既有人工确认）
...
- `0.15.0` 将主题、字模、Logo、布局、DTO 和逐行渲染移入 `app/ui/lcd`，LCD BSP 只保留控制器命令、
