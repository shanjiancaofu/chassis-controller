...
| 当前工作树 | 统一采样时间戳与轮式里程计 | `0.12.0 build1` | 65 mm 轮径、220 mm 轮距的里程计、UART/LCD 输出已构建，尚未上板 |
...
统一单调时间轴和 STM32 轮式里程计已在 `0.12.0 build1` 实现，当前等待架空轮 UART 符号/时间
检查和落地几何校准。Jetson 后续消费轮式里程计和 IMU 输出并负责 ROS 2 定位融合，不在 STM32
