...
`flash_layout.h`：内部 Bootloader、内部单 Application、QSPI Metadata 和
QSPI Firmware Slot A/B 的地址与大小。不能复制其他 STM32 型号地址。

`firmware_image.h`：镜像类型、版本结构、硬件 ID、长度、SHA-256、签名和镜像头格式。只定义格式，不保存当前版本值。

`ota_metadata.h`：QSPI confirmed slot、candidate slot、boot state、trial boot count、
upgrade sequence 和 CRC。
...
进入 UPDATE 状态，关闭电机
  ↓
通过 CAN FD 或 UART DMA + 环形缓冲区接收固件
  ↓
写入 QSPI 非活动固件槽
  ↓
校验 CRC、SHA-256 和签名
...
重启
  ↓
Bootloader 从候选槽校验并安装到内部单 Application 区
  ↓
Bootloader 试运行新 Application
  ↓
Application 自检
  ↓
...
实现 V1 QSPI 固件槽 A/B、内部单 Application 安装、确认和回滚。
V2 再考虑 Bootloader CAN FD Recovery。
...
OTA V1：
Application 正式支持通过 CAN FD 或串口接收固件
串口使用 DMA circular + IDLE + 软件环形缓冲区，任务上下文解析和写入 QSPI
CAN FD 与串口复用同一 OTA 分包协议和状态机，同一时刻只允许一个传输源
CAN FD 和串口必须分别完成完整升级验收，二者互不替代
QSPI 保存已确认镜像和候选镜像
Bootloader 校验、安装、试运行确认，并在失败时恢复已确认镜像
