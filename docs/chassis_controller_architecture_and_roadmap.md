...
裸机基线 `v0.1.0-baremetal` 已完成板级外设、双电机、编码器、架空低速 PID、CAN FD 和安全链路验收。当前处于 FreeRTOS 调度迁移阶段，第一批已完成内核生成、静态应用任务接入和 GCC 构建，尚待烧录后的硬件回归。

后续主要工作包括：

- FreeRTOS 固件的完整硬件回归
- 独立 10 ms 控制任务和 TIM6 ISR 任务通知
- 通信接收与低优先级诊断任务边界
- Jetson 下发线速度、角速度
- 差速运动学
- 里程计或完整原始反馈链路
...
完整裸机 Demo 已完成，FreeRTOS 迁移已经开始。当前第一批采用 CubeMX CMSIS-RTOS2 接入内核，应用代码直接使用原生 FreeRTOS API；SysTick 用于内核节拍，TIM7 用于 HAL 时基，TIM6 保留 10 ms 底盘控制节拍。

原则：

> 只改变调度方式，不改变底盘行为。

第一批只用一个 1024 Words 静态应用任务承接原有 `App_Run()`，不提前把每个模块拆成任务。完成硬件回归后，再将 TIM6 ISR 改为发送任务通知，由独立高优先级控制任务执行 10 ms 控制周期。
...
进行中。第一批已完成内核接入、静态应用任务、TIM7 HAL 时基和 GCC 构建；下一步完成硬件回归，再迁移 10 ms 控制任务和通信/诊断任务边界。
